...
| GGML_CUDA_FA_ALL_QUANTS       | Boolean                | false   | Compile all 169 ordered CUDA FlashAttention vector pairs for the 13 retained standard cache types, plus all 36 KVarN fast-decode pairs. Use this for broad asymmetric-cache experiments. |

With no quant-matrix flag, CUDA FlashAttention compiles 103 vector pairs over `f16`, `bf16`, `q8_0`, `q6_1`, `q6_0`, `q5_1`, `q5_0`, `q4_1`, `q4_0`, `q3_1`, `q3_0`, `q2_1`, and the fork's internal q2 fallback type. The default predicate is `rank(K) <= rank(V) || K == f16 || V == f16`; the F16 axes cover materialized mixed-cache paths. Pairs outside the vector matrix use the normal CUDA FlashAttention fallback rather than an uncompiled dispatch case. KVarN keeps 15 balanced fast-decode pairs by default and all 36 with `GGML_CUDA_FA_ALL_QUANTS=ON`; every valid KVarN bit pair remains supported through descriptor-native MMA when it is outside the fast matrix. `GGML_CUDA_FA_HALF_QUANTS` has been removed.
