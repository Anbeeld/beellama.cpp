...
| GGML_CUDA_FA_ALL_QUANTS       | Boolean                | false   | Compile CUDA FlashAttention vec kernels for the full supported K/V cache type matrix: 361 pairs for the 19-type universe, plus all 36 KVarN fast decode pairs. This is useful for arbitrary asymmetric cache-type combinations. Mutually exclusive with GGML_CUDA_FA_HALF_QUANTS.                                                                                                         |
| GGML_CUDA_FA_HALF_QUANTS      | Boolean                | false   | Compile the larger CUDA FlashAttention vec K/V cache pair set for this fork: the useful K>=V half-matrix plus f16 fallback pairs needed by TurboQuant/TCQ dequant paths. This compiles 217 pairs instead of 361 in GGML_CUDA_FA_ALL_QUANTS, plus the 21 KVarN K>=V fast decode pairs. Types are ranked from higher precision to lower: f16 > bf16 > q8_0 > q6_1 > q6_0 > q5_1 > q5_0 > q4_1/turbo4_tcq/turbo4 > q4_0 > q3_1/turbo3_tcq/turbo3 > q3_0 > q2_1/turbo2_tcq/turbo2 > q2_0. Pseudo-equal qX_1/Turbo/TurboTCQ groups compile both directions; qX_0-to-Turbo pairs are excluded as K<V. Mutually exclusive with GGML_CUDA_FA_ALL_QUANTS. |

Recommended CUDA FlashAttention builds should leave both quant flags off. The default compiles 62 much faster q/KVarN-fallback pairs for standard q cache types or KVarN. It excludes Turbo/TCQ and mixed f16/bf16 pairs. For standard q and KVarN fallback types, a pair is compiled only when K is the same or higher precision than V and V is no more than two tier groups below K. Tier groups are fp (`f16`, `bf16`), high q (`q8_0`, `q6_1`, `q6_0`), q5, q4, q3, and q2; within a bit width, `_1` ranks above `_0`. TurboQuant/TCQ has not shown a benchmark advantage over standard q or KVarN cache types in current fork benchmarks. The same flags select KVarN fast decode pairs: 15 balanced K>=V pairs by default, all 21 K>=V pairs with `GGML_CUDA_FA_HALF_QUANTS=ON`, and all 36 ordered pairs with `GGML_CUDA_FA_ALL_QUANTS=ON`. Every KVarN pair remains supported in the default build; pairs outside its fast matrix use the descriptor-native MMA fallback. The default policy uses same-or-higher precision K because K loses precision faster under quantization and lower-precision K than V is inefficient, but avoids K far above V because large K/V tier gaps are unbalanced. Use `GGML_CUDA_FA_HALF_QUANTS=ON` when you want TurboQuant/TCQ or high-delta K>=V experiments, and `GGML_CUDA_FA_ALL_QUANTS=ON` only when the full 361-pair matrix is required.
