...
// The retained 32-element q2 cache formats accumulate more dot-product error than
// the larger upstream low-bit blocks, while still matching their reference paths.
constexpr float MAX_DOT_PRODUCT_ERROR_2BITS = 0.15f;
    ...
    for (int i = 0; i < GGML_TYPE_COUNT; i++) {
        ...
        if (qfns_cpu->from_float && qfns->to_float) {
            ...
            const float max_quantization_error =
                ...
                type == GGML_TYPE_Q2_0    ? MAX_QUANTIZATION_TOTAL_ERROR_2BITS :
                type == GGML_TYPE_Q2_1    ? MAX_QUANTIZATION_TOTAL_ERROR_2BITS :
                type == GGML_TYPE_IQ2_S   ? MAX_QUANTIZATION_TOTAL_ERROR_2BITS :
                type == GGML_TYPE_Q3_K    ? MAX_QUANTIZATION_TOTAL_ERROR_3BITS :
                type == GGML_TYPE_Q3_0    ? MAX_QUANTIZATION_TOTAL_ERROR_3BITS :
                type == GGML_TYPE_Q3_1    ? MAX_QUANTIZATION_TOTAL_ERROR_3BITS :
            ...
            if (qfns_cpu->vec_dot) {
                ...
                const float max_allowed_error = type == GGML_TYPE_Q2_0 || type == GGML_TYPE_Q2_1
                                              ? MAX_DOT_PRODUCT_ERROR_2BITS
                ...
                printf("%5s dot product error:              skipped (no CPU vec_dot)\n", ggml_type_name(type));
            }
        }
    }
