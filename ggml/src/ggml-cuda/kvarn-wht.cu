...
void ggml_cuda_op_kvarn_wht(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    switch (head_width) {
        case 128:
            kvarn_wht_kernel<1><<<(int) n_groups, 128, 0, stream>>>(src_d, dst_d, n_groups);
            break;
        case 256:
            kvarn_wht_kernel<2><<<(int) n_groups, 128, 0, stream>>>(src_d, dst_d, n_groups);
            break;
        case 512:
            kvarn_wht_kernel<4><<<(int) n_groups, 128, 0, stream>>>(src_d, dst_d, n_groups);
            break;
        default:
