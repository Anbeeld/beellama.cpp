...
static void ggml_vk_gated_delta_net(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst) {
    ...
    const bool state_is_4d = src_state->ne[0] == S_v &&
                             src_state->ne[1] == S_v &&
                             src_state->ne[2] == H &&
                             src_state->ne[3] == n_seqs;
    // state is 3D (S_v*S_v*H, K, n_seqs); 4D state is the old K=1 cache layout.
    const uint32_t K = state_is_4d ? 1u : (uint32_t)src_state->ne[1];
...
static bool ggml_backend_vk_kvarn_native_ops(ggml_backend_dev_t dev) {
    UNUSED(dev);
    // Vulkan currently has a native store shader only. Do not advertise full
    // KVarN cache support until FLASH_ATTN_EXT can consume GGML_OP_KVARN_VIEW.
    return false;
}
