...
llm_build_qwen35::llm_build_qwen35(const llama_model & model, const llm_graph_params & params) :
    ...
    // Pre-norm hidden state — captured for MTP draft input when enabled
...
// MTP (Multi-Token Prediction) graph builder for Qwen3.5.
// Runs a single decoder block at the nextn layer index. The block takes
// hidden-state input from the target model (pre-norm) and token embeddings,
// projects them through eh_proj, runs gated attention + dense FFN, and
// outputs logits via a shared or dedicated LM head.
llm_build_qwen35_mtp::llm_build_qwen35_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    // The MTP block lives at the first nextn layer index
    ...
    // Input: token ids + hidden state from target model
    ...
    // Normalize hidden state and token embeddings, then project
    ...
    // Concat along embedding dimension: [2*n_embd, n_tokens]
    ...
    // --- Single decoder layer: gated attention + dense FFN ---
    ...
    // Gated Q projection: outputs [query, gate] interleaved
    const int64_t n_head_kv_il = hparams.n_head_kv(il);
    ...
    // Sigmoid gate (same pattern as trunk layers)
    ggml_tensor * gate_sigmoid = ggml_sigmoid(ctx0, gate);
    cur = ggml_mul(ctx0, cur, gate_sigmoid);
    ...
    // Pre-norm hidden state output (consumed by the next MTP call or discarded)
    res->t_h_pre_norm = cur;
