...
// MTP (Multi-Token Prediction) graph builder for Qwen3.5MoE.
// Same structure as qwen35_mtp but the FFN uses MoE routing (experts +
// shared experts) matching the trunk layer pattern at the nextn index.
llm_build_qwen35moe_mtp::llm_build_qwen35moe_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    // --- Single decoder layer: gated attention + MoE FFN ---
    ...
    ggml_tensor * ffn_inp = cur; // save for shared expert gating
    ...
    // Pre-norm hidden state for MTP carryover
    res->t_h_pre_norm = cur;
