...
std::vector<std::vector<uint32_t>> llama_kv_cache::state_v2_read_payload_and_install(
    ...
    if (seq_id == -1) {
        ...
        if (reference_only_partial) {
            // Overlay partial restore is a suffix rollback plus exact-tail reinstall.
            // The live body already holds the checkpoint prefix, so wiping the whole
            // sequence and reinserting every cell is both unnecessary and quadratic.
            llama_pos max_pos = -1;
            for (const auto & stream : manifest.streams) {
                for (const auto & saved : stream.cells) {
                    max_pos = std::max(max_pos, saved.pos);
                }
            }
            if (max_pos >= 0 && !seq_rm(seq_id, max_pos + 1, -1)) {
                throw std::runtime_error("failed to remove KV suffix for partial restore");
            }
            for (uint32_t s = 0; s < manifest.streams.size(); ++s) {
                const auto & saved_stream = manifest.streams[s].cells;
                restored_cells[s].reserve(saved_stream.size());
