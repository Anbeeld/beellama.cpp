...
std::vector<uint32_t> llama_kv_cache_head_split(
    ...
    if (weights.empty()) {
        throw std::invalid_argument("KV cache head split requires at least one device");
    }
    double total = 0.0;
    for (float weight : weights) {
        if (!std::isfinite(weight) || weight < 0.0f) {
            throw std::invalid_argument("KV cache head split weights must be finite and non-negative");
        }
        total += weight;
    }
    ...
    for (size_t i = 0; i < weights.size(); ++i) {
        ...
        const uint32_t high = i + 1 == weights.size() ? n_heads :
                total == 0.0 ? uint32_t(uint64_t(n_heads)*(i + 1)/weights.size()) :
                uint32_t(double(n_heads)*cumulative/total);
        result[i] = high - low;
        low = high;
    }
    return result;
